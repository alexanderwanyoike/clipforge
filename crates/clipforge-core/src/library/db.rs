...
mod tests {
    ...
    fn delete_removes_record() {
        let tmp = tempfile::tempdir().unwrap();
        let lib = Library::open(&tmp.path().join("lib.db")).unwrap();
        lib.insert(&sample_recording("d1", "To Delete", None))
            .unwrap();
    ...
    fn search_by_title() {
        let tmp = tempfile::tempdir().unwrap();
        let lib = Library::open(&tmp.path().join("lib.db")).unwrap();
        lib.insert(&sample_recording(
            "s1",
            "Epic Elden Ring Boss Fight",
            Some("Elden Ring"),
        ))
        .unwrap();
        lib.insert(&sample_recording(
    ...
    fn search_by_game_name() {
        let tmp = tempfile::tempdir().unwrap();
        let lib = Library::open(&tmp.path().join("lib.db")).unwrap();
        lib.insert(&sample_recording("g1", "Session One", Some("Baldurs Gate")))
            .unwrap();
        lib.insert(&sample_recording("g2", "Session Two", Some("Factorio")))
            .unwrap();
    ...
    fn search_no_matches_returns_empty() {
        let tmp = tempfile::tempdir().unwrap();
        let lib = Library::open(&tmp.path().join("lib.db")).unwrap();
        lib.insert(&sample_recording("n1", "Some Recording", None))
            .unwrap();
